...
import (
	...
	"middle/app/user/api/pkg/mail"
	"middle/app/user/api/pkg/sms"
	"middle/pkg/helpers"
	"strings"
	"sync"
)
...
type ConfigVerifyCode struct {
	CodeLength       int    `json:",default=5,options=5|6|7"` // 验证码长度
	ExpireTime       int64  `json:",default=15"`              // 验证码过期时间, 单位分钟
	DebugExpireTime  int64  `json:",default=10080"`           // 调试模式下验证码过期时间
...
func NewVerifyCode(serviceName string) *VerifyCode {
	once.Do(func() {
		internalVerifyCode = &VerifyCode{
			Store: &RedisStore{
				// 增加前缀保持数据库整洁，出问题调试时也方便
				KeyPrefix: serviceName + ":verifycode:",
			},
		}
	})
