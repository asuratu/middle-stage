...
var (
	...
	cacheMsPhonePrefix           = "cache:msUser:user:phone:"
)
...
// FindOneByPhone 只查询未被软删除的数据
func (m *defaultUserModel) FindOneByPhone(ctx context.Context, phone string) (*User, error) {
	msUserPhoneKey := fmt.Sprintf("%s%v", cacheMsPhonePrefix, phone)
	var resp User
	err := m.QueryRowCtx(ctx, &resp, msUserPhoneKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
		query := fmt.Sprintf("select %s from %s where `phone` = ? and del_state = ? limit 1", userRows, m.table)
		return conn.QueryRowCtx(ctx, v, query, phone, globalkey.DelStateNo)
	})
	switch err {
	case nil:
		return &resp, nil
