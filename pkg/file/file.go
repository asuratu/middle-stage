...
// SaveUploadAvatarOrigin 保存上传的原始头像
func SaveUploadAvatarOrigin(c *gin.Context, file *multipart.FileHeader) (string, error) {

	var avatar string
	// 确保目录存在，不存在创建
	publicPath := "public"
	dirName := fmt.Sprintf("/uploads/avatars/%s/", app.TimeNowInTimezone().Format("2006/01/02"))
