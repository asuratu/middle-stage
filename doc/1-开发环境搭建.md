...
本项目地址 :  https://github.com/Mikaelemmmm/go-zero-middle
...
![gozeromiddle](./images/1/gozeromiddle.png)

### 四、业务架构图
![gozeromiddle](./images/1/go-zero-middle-service.png)
...
```shell
$ git clone https://github.com/Mikaelemmmm/go-zero-middle
$ go mod tidy
```
...
```shell
$ docker network create go-zero-middle_middle_net
...
```shell
$ ./kafka-topics.sh --create --zookeeper zookeeper:2181 --replication-factor 1 -partitions 1 --topic middle-log
...
创建数据库middle_order && 导入deploy/sql/middle_order.sql数据

创建数据库middle_payment && 导入deploy/sql/middle_payment.sql数据

创建数据库middle_travel && 导入deploy/sql/middle_travel.sql数据

创建数据库middle_usercenter && 导入middle_usercenter.sql数据
...
然后在当前页面，Create index pattern->输入middle-*  -> Next Step ->选择@timestamp->Create index pattern
...
- kafka中没有创建log的topic ： middle-log

  解决：去kafka创建 middle-log，重启filebeat、go-stash
...
- 内部kafka问题
  ...
  $ docker exec -it kafka /bin/sh
  $ cd /opt/kafka/bin
  $ ./kafka-console-consumer.sh --bootstrap-server kafka:9092 --topic middle-log 
  ```
  ...
  ​		  2）在kafka容器内部命令行使用consumer.sh消费middle-log，另外一个终端命令行用producer.sh给middle-log发送消息，如果consumer收不到，说明kafka出问题了，docker logs -f kafka看看什么问题
